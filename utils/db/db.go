...
func execContext(ctx context.Context, name string, db unionDB, query Query, args []interface{}) (sql.Result, error) {
	...
	logger := log.Get(ctx)
	logger.WithField("cost", cost).Debugf("[DB] name:%s sql:%s args:%v", name, query.sql, args)
...
func (tx *Tx) log(ctx context.Context, msg string) {
	...
	logger := log.Get(ctx)
	if msg == "commit" {
		logger.Debugf("commit, total cost:%s", duration)
	} else {
		logger.Warnf("rollback, total cost:%s", duration)
	}
